...
require(
    ...
    function (ec) {
        ...
        var MPATH = "echarts/";//自定义地图geojson数据加载路径
        var cityMap = {
...
function mapDbClick(param) {
    ...
    if (!f) {
        getOrganizationIdByNamesAndDates();
        return;
    }
    nowOrgName = mt;
    cityCenterChange(mt);//组织机构改变加载该组织机构下的地图中心点信息
    getOrganizationIdByNamesAndDates();//组织机构改变重新初始化所有数据
    changeCityNum++;
...
function orgReturn() {
    ...
    if (orgNames.length == 1) {
        ...
        cityCenterChange(orgNames[(orgNames.length - 2)]);
        getOrganizationIdByNamesAndDates();//组织机构改变重新初始化所有数据
    }
}
