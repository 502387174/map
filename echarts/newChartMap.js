//取消炫光特效方法
function cancelCity() {
    if (markOrgNames == "" || markOrgNames == null) {
        return;
    }
    var cancelOrgs = markOrgNames.split(",");
    cancelOrgs.forEach(function (e) {
        if (e != "" && e != null) {
            myChart.delMarkPoint(0, e);
        }
    })
    markOrgNames = "";
}
...
require(
    ...
    function (ec) {
        ...
        var mapHeight = $(document).height() - 240;
        if ($(".mapMod").height() == 0) {
            $('#mapMain').height(mapHeight);
        } else {
            $('#mapMain').height($(".mapMod").height());
        }
        ...
        myChart.on(ecConfig.EVENT.DBLCLICK, function (param) {
            ...
            nowOrgName = mt;
//				cityCenterChange(mt);//组织机构改变加载该组织机构下的地图中心点信息
            changeCityNum++;
            option.series[0].mapType = mt;//显示选中区域地图
            getOrgByOrgName();
            if (linkType == "hidden") {
                initHiddenAllData();
            } else if (linkType == "exception") {
                initAllExceptionData();
            } else if (linkType == "float") {
                initFloatAllData();
            } else if (linkType == "special") {
                initSpecialAllData();
            }
        ...
        //因可能组织机构字段不会初始化，所以这里判断是否是有，隔开的组织机构，如果有，则取第一个组织机构名称
        if (mapTypeToUserOrg.indexOf(",") != -1) {
            mapTypeToUserOrg = mapTypeToUserOrg.split(",")[0] == "四川省" ? "四川" : mapTypeToUserOrg.split(",")[0];
        }
        option = {
            tooltip: {
...
$(function () {
    ...
    $("#returnBtn").click(function () {
        var orgNames = organizationName.split(",");
        if (orgNames.length == 1) {
            return;
        } else {
            var orgBackName = orgNames[(orgNames.length - 2)];
            if (orgBackName == "四川省") {
                orgBackName = "四川";
            }
            option.series[0].mapType = orgBackName;
            myChart.restore();
            myChart.setOption(option, true);
            organizationName = organizationName.replace("," + orgNames[(orgNames.length - 1)], "");
            changeCityNum--;
            nowOrgName = orgNames[(orgNames.length - 2)];
            getOrgByOrgName();
            if (linkType == "hidden") {
                initHiddenAllData();
            } else if (linkType == "exception") {
